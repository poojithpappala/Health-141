...
const Index = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="hero-section hero-pattern relative overflow-hidden animate-section-fade">
        ...
        <div className="section-container relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            <Badge className="bg-wellness-gold/10 text-wellness-gold border-wellness-gold/30 mb-8 px-8 py-3 rounded-full text-lg font-semibold hover:bg-wellness-gold/20 transition-colors">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center mb-16">
              <Button 
                asChild 
                size="lg" 
                className="brand-gradient text-primary-foreground font-semibold btn-hover premium-shadow text-lg px-10 py-3"
              >
              ...
              <Button 
                ...
                variant="outline" 
                size="lg" 
                className="font-semibold btn-hover premium-shadow text-lg border-[hsl(var(--primary))] text-[hsl(var(--primary))] hover:text-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))]/10 px-10 py-3"
              >
                <Link to="/services">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {stats.map((stat, index) => (
                <div key={index} className="text-center animate-fade-in" style={{animationDelay: `${index * 0.15}s`}}>
                  <div className={`w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center brand-gradient shadow-lg group transition-all duration-300 hover:scale-110`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (
              <div key={index} className="card-premium p-8 text-center group animate-fade-in" style={{animationDelay: `${0.2 + index * 0.15}s`}}>
                <div className="w-20 h-20 mx-auto mb-6 brand-gradient rounded-3xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg">
                  <feature.icon className="w-10 h-10 text-white" />
                </div>
                <h3 className="text-2xl font-bold text-wellness-charcoal mb-4 group-hover:text-[hsl(var(--primary))] transition-colors">{feature.title}</h3>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            {services.map((service, index) => (
              <div key={index} className="card-premium p-8 group hover:scale-105 transition-all duration-300 animate-fade-in" style={{animationDelay: `${0.4 + index * 0.15}s`}}>
                <div className={`w-20 h-20 bg-gradient-to-br ${service.color} rounded-3xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 shadow-md`}>
                  <service.icon className="w-10 h-10 text-white" />
                </div>
                
                <h3 className="text-2xl font-bold text-wellness-charcoal mb-3 group-hover:text-[hsl(var(--primary))] transition-colors">{service.title}</h3>
      ...
      {/* CTA Section */}
      <section className="content-section brand-gradient relative overflow-hidden animate-section-fade">
        <div className="absolute inset-0 hero-pattern opacity-10" />
        <div className="section-container relative z-10 text-center">
          <h2 className="text-5xl lg:text-6xl font-bold text-white mb-8 leading-tight">
            Ready to Transform Your Health?
          </h2>
          <p className="text-xl text-teal-50 mb-12 max-w-3xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center">
            <Button 
              asChild 
              size="lg" 
              className="bg-white text-[hsl(var(--primary))] hover:bg-slate-100 font-bold text-lg btn-hover premium-shadow px-10 py-3"
            >
              <Link to="/patient-intake">
                <Calendar className="mr-3 w-5 h-5" /> 
            ...
            <Button 
              ...
              variant="outline" 
              size="lg" 
              className="border-2 border-white text-white hover:bg-white/10 backdrop-blur-sm font-bold text-lg btn-hover px-10 py-3"
            >
              <Link to="/contact">
